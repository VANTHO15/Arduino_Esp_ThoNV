...
void WiFiManager::handleRoot() {
  ...
  page += String(F("<h1>"));
  page += F("Setting WiFi And Token Blynk");
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  if (scan) {
    ...
    if (n == 0) {
      ...
      page += F("WI-FI сети не найдены. ќбновите страницу дл€ повторного сканировани€.");
    } else {
...
void WiFiManager::EC_save(){
   ...
   size_t sz1 = sizeof(authtok);
   for( int i=0; i<sz1; i++)
      EEPROM.write(i, *((uint8_t*)&authtok+ i));
   EEPROM.commit(); 
...
strcpy(authtok, server->arg("t").c_str());
...
String page = FPSTR(HTTP_HEAD);
  ...
  page += "<br/><form action=\"/\" method=\"post\"><button>Back</button></form>";
    page += FPSTR(HTTP_END);
  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);
  EC_save();
...
uint16_t WiFiManager::EC_SRC(){
   uint16_t src = 0;
   size_t sz1 = sizeof(authtok);
   uint16_t src_save = STRC;
   STRC = 0;
   for( int i=0; i<sz1; i++)src +=*((uint8_t*)&authtok+ i);
   STRC = src_save;
   return src;  
   //DEBUG_WM(src);
}
...
void WiFiManager::handleUpdate() {
  ...
  page += "<br/><form action=\"/\" method=\"post\"><button>Return</button></form>";
  page += FPSTR(HTTP_END);
...
void WiFiManager::handleReset() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEAD_END);
  page += F("—ейчас устройство перезагрузитс€.");
  page += FPSTR(HTTP_END);
